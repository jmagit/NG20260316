import { FilterPipe, OrderByPipe } from './colecciones';
...
describe('FilterPipe', () => {
  ...
  it('Should return all, with no predicate', () => {
    const result = pipe.transform(people);
    expect(result).toEqual(people);
  });

  it('Should return everything, with predicate always true', () => {
    const result = pipe.transform(people, () => true);
    expect(result).toEqual(people);
  });

  it('Should return everything, with firstName starts with "L"', () => {
    const result = pipe.transform(people, item => item.firstName.startsWith('L'));
    expect(result).toEqual([people[0],people[1],]);
  });

  it('Should not return everything, with predicate always false', () => {
    const result = pipe.transform(people, () => false);
   expect(result.length).toEqual(0);
  });

  it('Should not return everything, with age less than 18', () => {
    const result = pipe.transform(people, item => item.age < 18);
   expect(result.length).toEqual(0);
  });
})
...
describe('OrderByPipe', () => {
  ...
  it('Should return the persons ordered by last name (asc)', () => {
    ...
    expect(result).toEqual([people[3], people[4], people[1], people[6], people[5], people[0], people[2]]);
  });

  it('Should return the persons ordered by last name (desc)', () => {
    const result = pipe.transform(people, '-lastName');
    expect(result).toEqual([people[2], people[0], people[5], people[6], people[1], people[4], people[3]]);
  });
  ...
  it('Should return the persons ordered by age (desc) and firstName (asc)', () => {
    ...
    expect(result).toEqual([people[4], people[1], people[5], people[0], people[6], people[3], people[2]]);
  });
